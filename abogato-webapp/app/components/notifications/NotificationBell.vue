...
async function handleToggleType(type: NotificationType) {
  const updated = await setTypeEnabled(
    type,
    !enabledTypes.value.includes(type),
  );
  ...
  <div ref="root" class="relative">
    <div class="relative">
      <UButton
        ...
        square
        class="h-10 w-10 rounded-xl border-default/80 bg-default/90 shadow-sm sm:h-11 sm:w-11"
    ...
    <Teleport to="body">
      ...
      >
        <div v-if="isOpen" class="fixed inset-0 z-[510] pointer-events-none">
          <div
            v-if="showBackdrop"
            class="absolute inset-0 bg-black/30 backdrop-blur-[1px] pointer-events-auto"
            @click="handleBackdropClick"
          />

          <div
            ref="panel"
            class="fixed z-[520] pointer-events-auto"
          ...
          >
            <NotificationsNotificationPanel
              ...
              :error="lastError"
              :is-mobile="isMobile"
              @drag-start="!isMobile ? startDragging($event) : undefined"
              @select="handleSelect"
